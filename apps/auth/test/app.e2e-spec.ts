...
describe('Auth (microservice e2e)', () => {
  ...
  it('should not sign up with short password', async () => {
    await expect(
      firstValueFrom(
        client.send(
          { cmd: 'auth.user.signup' },
          { email: 'test3@example.com', password: '123' },
        ),
      ),
    ).rejects.toEqual(expect.anything());
  });
  ...
  it('should change user role', async () => {
    const userRes = await firstValueFrom(
      client.send(
        { cmd: 'auth.user.signup' },
        { email: 'biztest1@example.com', password: 'password123' },
      ),
    );
    const userId = userRes._id || userRes.id;
    const res = await firstValueFrom(
      client.send(
        { cmd: 'auth.user.change-role' },
  ...
  it('should return error for already same role', async () => {
    const userRes = await firstValueFrom(
      client.send(
        { cmd: 'auth.user.signup' },
        { email: 'biztest2@example.com', password: 'password123' },
      ),
    );
    const userId = userRes._id || userRes.id;
    await expect(
      firstValueFrom(
        client.send({ cmd: 'auth.user.change-role' }, { userId, role: 'USER' }),
      ),
  ...
  it('should return error for invalid role', async () => {
    const userRes = await firstValueFrom(
      client.send(
        { cmd: 'auth.user.signup' },
        { email: 'biztest3@example.com', password: 'password123' },
      ),
    );
    const userId = userRes._id || userRes.id;
    await expect(
